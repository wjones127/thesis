...
#' prob_var_plot(test_data, "x", "y", window_width = 200, rug = FALSE)

prob_var_plot <- function(data, x_var, y_var, method="exact", n = 200,
                          window_width = NULL, rug = TRUE) {
...
#  print(conf_int)
  ...
  if (rug) {
    p <- p +     geom_point(shape=124, aes(x = orig_data[[x_var]],
                                           y = orig_data[[y_var]]),
                            data = orig_data)
  }
  p

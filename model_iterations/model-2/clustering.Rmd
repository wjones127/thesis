...
length_scree <- qplot(x = 1:bins, y = pc_length$sdev^2 / sum(pc_length$sdev^2)) + 
  geom_line() + 
...
ggsave("plots/length_loadings.pdf", width = 3, height = 2)
...
time_scree <- qplot(x = 1:(ncol(rider_times2)-1), y = pc2$sdev^2 / sum(pc2$sdev^2)) + 
  geom_line() + 
...
ggsave("plots/time_loadings.pdf", width = 4, height = 4)
...
```{r clustering}
riders_final <- riders %>%
  left_join(rider_times_w_pc2, by = "rider") %>%
...
ggsave("plots/choice_k.pdf", width = 3, height = 2)
```


```{r}
rides_scaled <- select(riders_final, rider, cluster) %>%
  right_join(rides_scaled, by = "rider")

cluster_patterns <- rides_scaled %>%
  filter(weekend == "FALSE") %>%
ggplot(aes(x = time, y = log_length)) + 
  ...
  labs(x = "time of day", y = "log(length)", 
       title = "Patterns in Ride Length and Time of Day for Clusters")

cluster_patterns

ggsave("plots/cluster_patterns.pdf", cluster_patterns, width = 5, height = 3)

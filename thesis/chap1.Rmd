...
and (somewhat) reliable human input in favor of increasing sample size (i.e.
...
are given more direction in what they are rating for.) This leads to some 
potential issues that we need to address in our models.

<!-- how does it work? -->
Before we get into the potential issues in the data collection, let's examine
...
app does allow riders to correct the misclassification, but there is currently
no way to join split rides back together (Knock is working on changing that, 
though.)
...
version as of Februrary 2015.", 
      label = "ride-rating", type = "figure", scale=.35,
options = "tbh")
```
...
fundamentals of the data collection process, have remained constant, however.
...
weather_station_map <- ggmap(portland_map) + 
  ...
  geom_label(aes(x = long, y = lat, label = label), 
             data = weather_stations,
             nudge_y = -0.007) +
  labs(x = "longitude", y = "latitude")

ggsave("figure/weather_station_map.pdf", weather_station_map)

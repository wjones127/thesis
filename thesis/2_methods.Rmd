...
to as hierachical models or mixed effects models.] models, additive models,
...
The one large methology not covered here is the expectation maximization algorithm
...
package in \textit{R} or can be fit with Bayesian MCMC using \textit{Stan}.
...
Instead of focusing on one measure of fit we use several. Loglikelihood and AIC
...
low probability events? The Heatmap plot, another graphic tool we assess models
with, show use how well the predicted probabilities fit the empirical probabilities
of the corresponding observations; i.e. if the model says a set of observations
will have $y_i = 1$ 60 percent of the time, is the percentage of those that have
$y_i = 1$ equal to 60?

We also use area under the ROC curve (AUC) measure popular for assessing logistic

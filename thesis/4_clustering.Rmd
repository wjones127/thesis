...
that differentiate types of riders and then use these variables
...
clustering. To choose the number of clusters,
...
makes sense, given `r ref("choosing_k")` only showed small reductions in total within
sum of squares as $k$ got larger. Despite having failed to identify distinct
clusters, studying these different groups still offers some valuable insight
...
If we look at the length and time patterns of the clusters, shown in 
`r ref("cluster-patterns")`, it's clear that these aggregate patterns give some
...
cluster 2 having more consistently long weekend rides. 
...
introducing rider intercepts? Because a model with only 4 random intecepts---as
